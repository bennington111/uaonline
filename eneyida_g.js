...
(function () {
    ...
    async function searchM3u8(url, depth = 0) {
        ...
        console.log(`[Eneyida Universal] Запит сторінки: ${url} (глибина ${depth})`);

        const resp = await fetch(proxy + url);
        const html = await resp.text();

        console.log(`[Eneyida Universal] Отримано HTML з ${url}:\n`, html.substring(0, 1000));
        ...
        for (const script of scripts) {
            ...
            if (m3u8Match) {
                console.log(`[Eneyida Universal] Знайдено m3u8 у script:`, m3u8Match[0]);
                return m3u8Match[0];
            }
        }
        ...
        if (m3u8MatchInHtml) {
            console.log(`[Eneyida Universal] Знайдено m3u8 у HTML:`, m3u8MatchInHtml[0]);
        ...
        if (iframe) {
            ...
            if (iframeUrl) {
                console.log(`[Eneyida Universal] Переходимо в iframe: ${iframeUrl}`);
    ...
    async function loadOnline(movie) {
        ...
        try {
            ...
            console.log('[Eneyida Universal] Знайдено посилання на відео:', videoUrl);
