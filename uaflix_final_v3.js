// ==UserScript==
// @name         Uaflix for Lampa (FINAL)
// @namespace    https://github.com/bennington111/
// @version      4.6
// @description  Uaflix plugin for Lampa
// @author       Bennington
// @match        *://lampa.mx/*
// @icon         https://uafix.net/favicon.ico
// @grant        none
// @run-at       document-end
// ==/UserScript==

(function() {
    'use strict';
    
    const mod_version = '4.6';
    
    // Ваш оригінальний HTML кнопки
    const button_html = `
    <div class="full-start__button selector view--uaflix" data-subtitle="uaflix ${mod_version}">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 244 260" width="24" height="24" fill="currentColor">
            <path d="M242,88v170H10V88h41l-38,38h37.1l38-38h38.4l-38,38h38.4l38-38h38.3l-38,38H204L242,88L242,88z
            M228.9,2l8,37.7l0,0L191.2,10L228.9,2z M160.6,56l-45.8-29.7l38-8.1l45.8,29.7L160.6,56z
            M84.5,72.1L38.8,42.4l38-8.1l45.8,29.7L84.5,72.1z M10,88L2,50.2L47.8,80L10,88z"/>
        </svg>
        <span>UAFlix</span>
    </div>`;

    // Функція додавання кнопки
    function addButton() {
        // Шукаємо контейнер меню
        const menu = document.querySelector('.full-start__scroll, .full-start__buttons');
        if (menu && !document.querySelector('.view--uaflix')) {
            const button = document.createElement('div');
            button.innerHTML = button_html;
            button.querySelector('.view--uaflix').addEventListener('click', function() {
                const title = document.querySelector('.card__title')?.textContent || '';
                const year = document.querySelector('.card__year')?.textContent || '';
                window.open(`https://uafix.net/index.php?do=search&subaction=search&story=${encodeURIComponent(title + ' ' + year)}`);
            });
            menu.appendChild(button);
            return true;
        }
        return false;
    }

    // Головна функція ініціалізації
    function init() {
        // Спроба додати кнопку одразу
        if (!addButton()) {
            // Якщо не вийшло - спостерігаємо за змінами DOM
            const observer = new MutationObserver(function() {
                if (addButton()) {
                    observer.disconnect();
                }
            });
            
            observer.observe(document.body, {
                childList: true,
                subtree: true
            });
            
            // Зупинити спостерігання через 10 секунд
            setTimeout(() => observer.disconnect(), 10000);
        }
    }

    // Запускаємо
    if (document.readyState === 'complete') {
        setTimeout(init, 1000);
    } else {
        window.addEventListener('load', () => setTimeout(init, 1000));
    }
})();
