...
namespace Uaflix.Controllers
{
    ...
    public class UaflixController : BaseController
    {
            ...
            string source = "")
        {
            ...
            string memKey = $"UaFlix:view:{kinopoisk_id}:{imdb_id}:{title}:{original_title}";
            ...
            {
                Log($"Cache miss for key: {memKey}");
                result = await Search(imdb_id, kinopoisk_id, title, original_title, serial);
                if (result != null)
                {
                    hybridCache.Set(memKey, result, cacheTime(5));
                    Log($"Cached result for key: {memKey}");
                }
            }
            else
            {
                Log($"Cache hit for key: {memKey}");
            }

            // Перевіряємо чи є результати пошуку для відображення
            if (result?.searchResults != null && result.searchResults.Count > 0)
            {
                // Перевіряємо чи це дійсно пошуковий запит (без додаткових параметрів)
                if (string.IsNullOrEmpty(t) && string.IsNullOrEmpty(s) && string.IsNullOrEmpty(e))
                {
                    Log($"Index method - Found search results: {result.searchResults.Count} items");
                    // Відображаємо список результатів пошуку
                    var stpl = new SimilarTpl(result.searchResults.Count);
                    
                    foreach (var searchResult in result.searchResults)
                    {
                        Log($"Index method - Processing search result: title='{searchResult.title}', year='{searchResult.year}', poster='{searchResult.poster}'");
                        string link = $"{Request.Scheme}://{Request.Host}/uaflix?id={id}&imdb_id={imdb_id}&kinopoisk_id={kinopoisk_id}&title={HttpUtility.UrlEncode(searchResult.url)}&original_title={HttpUtility.UrlEncode(searchResult.url)}&year={HttpUtility.UrlEncode(searchResult.year)}";
                        
                        stpl.Append(searchResult.title, searchResult.year, string.Empty, link, searchResult.poster);
                    }
                    
                    string htmlContent = stpl.ToHtml();
                    Log($"Index method - Generated HTML length: {htmlContent.Length}");
                    Log($"Index method - HTML preview: {htmlContent.Substring(0, Math.Min(200, htmlContent.Length))}...");
                    
                    return Content(htmlContent, "text/html; charset=utf-8");
                }
                else
                {
                    Log($"Index method - Not a search request, parameters: t='{t}', s='{s}', e='{e}'");
                }
            ...
            // Якщо результат null - показуємо повідомлення про відсутність результатів
            if (result == null)
            {
                return Content("Пошук не дав результатів.", "text/html; charset=utf-8");
            }
        ...
        async ValueTask<Result> Search(string imdb_id, long kinopoisk_id, string title, string original_title, int serial)
        {
            ...
            Log($"Search method - title='{title}', original_title='{original_title}', filmTitle='{filmTitle}'");
            
            // Перевіряємо чи title містить URL (це означає що це результат пошуку)
            if (filmTitle.StartsWith("http") || filmTitle.Contains("uafix.net"))
            {
                Log($"Direct URL detected: {filmTitle}");
                // Це прямий URL, обробляємо його безпосередньо
                try
                {
                    var filmHtml = await httpClient.GetStringAsync(filmTitle);
                ...
                {
                    Log($"Error processing direct URL: {ex.Message}");
                }
            }
            ...
            try
            {
                ...
                if (filmNodes == null || !filmNodes.Any()) return null;

                // Якщо знайдено кілька результатів - повертаємо список
                if (filmNodes.Count > 1)
                {
                    ...
                    foreach (var filmNode in filmNodes)
                    {
                        ...
                        var imgNode = filmNode.SelectSingleNode(".//img");
                        ...
                        // Парсимо назву з різних можливих місць
                        string searchResultTitle = "";
                        var titleNode = filmNode.SelectSingleNode(".//h2") ?? 
                                       filmNode.SelectSingleNode(".//div[contains(@class, 'title')]") ??
                                       filmNode.SelectSingleNode(".//div[contains(@class, 'name')]");
                        if (titleNode != null)
                            searchResultTitle = titleNode.InnerText?.Trim() ?? "";

                        // Парсимо рік
                        string year = "";
                        var yearMatch = Regex.Match(searchResultTitle, @"\((\d{4})\)");
                        if (yearMatch.Success)
                        {
                            year = yearMatch.Groups[1].Value;
                            searchResultTitle = searchResultTitle.Replace(yearMatch.Value, "").Trim();
                        }

                        // Парсимо опис
                        string description = "";
                        var descNode = filmNode.SelectSingleNode(".//div[contains(@class, 'desc')]") ??
                                     filmNode.SelectSingleNode(".//div[contains(@class, 'text')]") ??
                                     filmNode.SelectSingleNode(".//p");
                        if (descNode != null)
                    ...
                    // Повертаємо результат з результатами пошуку
                    return new Result { searchResults = searchResults };
                }

                // Якщо знайдено тільки один результат - обробляємо його
                var singleFilmNode = filmNodes.First();
                string singleFilmUrl = singleFilmNode.GetAttributeValue("href", "");
                if (!singleFilmUrl.StartsWith("http"))
                    singleFilmUrl = "https://uafix.net" + singleFilmUrl;

                var filmHtml = await httpClient.GetStringAsync(singleFilmUrl);
                doc.LoadHtml(filmHtml);
